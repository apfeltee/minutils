...
static enum test_type get_test_type_long(const char* flag)
{
    if(!strcmp(flag, "-eq"))
        ...
        return TEST_INT_LE;
    else if(!strcmp(flag, "-ef"))
        return TEST_SAMEFILE;
    else if(!strcmp(flag, "-nt"))
        return TEST_NEWER;
    else if(!strcmp(flag, "-ot"))
        return TEST_OLDER;
    else
        return TEST_INVALID;
}
...
static int file_cmp(const struct test_action* act)
{
    ...
    {
        case TEST_SAMEFILE:
            return stbuf_a.st_ino == stbuf_b.st_ino && stbuf_a.st_dev == stbuf_b.st_dev;
        case TEST_NEWER:
...
/* Integer operations */
static int get_ints(long* x, long* y, const struct test_action* act)
{
    char* ptr;
...
int main(int argc, char* argv[])
{
    ...
    switch(argc)
    {
        ...
        case 4:
            ...
            if(act.type == TEST_INVALID)
            {
                fprintf(stderr, "%s: unknown flag: '%s'", argv0, argv[2]);
                return -1;
            }
            break;
        /* Even more... */
