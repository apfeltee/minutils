...
static void print_statblock(const char* path, const struct stat* stbuf)
{
    ...
    printf("  File: %s\n"
           ...
           "Change: %s\n"
           " Birth: %s\n",
           path, (long)stbuf->st_size, (long)stbuf->st_blocks, (long)stbuf->st_blksize, typename, "???", (unsigned)stbuf->st_dev,
           (unsigned long)stbuf->st_ino, (long)stbuf->st_nlink, deviceinf, stbuf->st_mode & 07777, mode, stbuf->st_uid,
           (pwd) ? pwd->pw_name : "???", stbuf->st_gid, (grp) ? grp->gr_name : "???", atime, mtime, ctime, "-");
}
