...
MINISTARTMAIN(sort)
{
    ...
    {
        files.allocd = 0;
        files.array = (FILE**)malloc((1 * sizeof(FILE*)) + 1);
        files.array[0] = stdin;
        files.len = 1;
    }
    else
    {
        ...
        files.array = (FILE**)malloc(files.len * sizeof(FILE*));
        if(!files.array)
        {
    ...
    qsort(lines.array, lines.len, sizeof(char*), (opt.reverse) ? compare_reverse : compare_normal);
